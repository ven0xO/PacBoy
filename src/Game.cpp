...
namespace
{
    ...
    constexpr std::array<Type, 4> ENEMY_TYPES{Type::Red, Type::Pink, Type::Blue, Type::Orange};

    std::array<glm::vec2, 4> getEnemySpawnPositions(const Grid& grid)
    {
        return {grid.getRedGhostSpawnPosition(), grid.getPinkGhostSpawnPosition(),
...
Game::Game(const std::vector<std::string>& level_paths) : levelPaths(level_paths)
{
    ...
    player = std::make_unique<Player>(pacmanStart.x, pacmanStart.y, gameState);

    const auto spawnPositions = getEnemySpawnPositions(gameGrid);

    for (std::size_t index = 0; index < enemies.size(); ++index)
    {
        enemies[index] =
            std::make_unique<Enemy>(ENEMY_TYPES[index], gameGrid, *player, spawnPositions[index]);
...
{
    if (phase == GamePhase::Ready && readyTimer > 0.0f && currentFrame >= readyTimer)
    {
...
{
    if (phase == GamePhase::Playing && gameState.checkIfNextLevel())
    {
    ...
    if (phase == GamePhase::LevelComplete && currentFrame >= levelCompleteUntil)
    {
...
void Game::handleEnemyCollisions(float currentFrame)
{
    ...
    const bool hitByDangerousGhost =
    ...
    if (hitByDangerousGhost && gameplayTimer >= invulnerableUntil)
    {
        gameState.loseLife();
    ...
    for (const auto& enemy : enemies)
    {
        if (enemy->get_state() == State::Scared && enemy->checkCollision(playerRect))
        {
...
void Game::processPlayerInput(const GameInput& input, float currentFrame)
{
    ...
    else if (phase == GamePhase::NewScore)
    {
        ...
        if (keyBackspace && !prevKeyBackspace && !enteredName.empty())
        {
            enteredName.pop_back();
        }

        if (keyEnter && !prevKeyEnter && !enteredName.empty())
        {
...
{
    if (levelPaths.empty() || !gameGrid.loadFromFile(levelPaths.front()))
    {
...
bool Game::loadNextLevel(float currentFrame)
{
    ...
    const std::size_t nextLevelIndex = (currentLevelIndex + 1) % levelPaths.size();
