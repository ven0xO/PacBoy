...
{
    drawRectangle(0.0f, 0.0f, static_cast<float>(width), 38.0f, glm::vec3(0.0f, 0.0f, 0.0f));

    renderText("SCORE " + std::to_string(state.getScore()), 12.0f, 8.0f, 3.0f);

    renderText("LIVES " + std::to_string(state.getLives()), 300.0f, 8.0f, 3.0f);

    renderText("LEVEL " + std::to_string(state.getLevel()), 600.0f, 8.0f, 3.0f);
}
...
void Hud::renderMainMenu(int selected)
{
    ...
    const auto centeredX = [this, &textWidth](const std::string& text, float scale)
    { return (static_cast<float>(width) - textWidth(text, scale)) / 2.0f; };

    drawRectangle(50.0f, 25.0f, 700.0f, 550.0f, blue);

    drawRectangle(54.0f, 29.0f, 692.0f, 542.0f, panelColor);

    drawRectangle(88.0f, 101.0f, 115.0f, 3.0f, blue);

    drawRectangle(597.0f, 101.0f, 115.0f, 3.0f, blue);

    const std::string title{"PACBOY"};
    constexpr float titleScale{9.0f};
    const float titleX = centeredX(title, titleScale);

    renderText(title, titleX + 4.0f, 69.0f, titleScale, darkBlue);

    renderText(title, titleX, 65.0f, titleScale, yellow);

    const std::string subtitle{"ARCADE EDITION"};
    constexpr float subtitleScale{2.5f};

    renderText(subtitle, centeredX(subtitle, subtitleScale), 145.0f, subtitleScale, lightGray);

    drawRectangle(145.0f, 195.0f, 510.0f, 235.0f, darkBlue);

    drawRectangle(149.0f, 199.0f, 502.0f, 227.0f, glm::vec3(0.012f, 0.03f, 0.09f));
    ...
    for (int option = 0; option < 2; option++)
    {
        ...
        if (isSelected)
        {
            drawRectangle(172.0f, optionY - 13.0f, 456.0f, 58.0f, darkBlue);

            drawRectangle(172.0f, optionY - 13.0f, 456.0f, 2.0f, yellow);

            drawRectangle(172.0f, optionY + 43.0f, 456.0f, 2.0f, yellow);
        }

        renderText(optionText, optionX, optionY, optionScale, isSelected ? yellow : lightGray);

        if (isSelected)
        {
            renderText(">", optionX - 42.0f, optionY, optionScale, yellow);
    ...
    renderText(navigationHint, centeredX(navigationHint, hintScale), 460.0f, hintScale, lightGray);

    renderText(confirmHint, centeredX(confirmHint, hintScale), 490.0f, hintScale, blue);

    renderText(exitHint, centeredX(exitHint, hintScale), 535.0f, hintScale, mutedGray);
}
...
void Hud::renderReady()
{
    ...
    drawRectangle(135.0f, 205.0f, 530.0f, 190.0f, yellow, 0.95f);

    drawRectangle(139.0f, 209.0f, 522.0f, 182.0f, darkBlue, 0.92f);

    const std::string title{"READY!"};
    constexpr float titleScale{8.0f};
    const float titleX = centeredX(title, titleScale);

    renderText(title, titleX + 4.0f, 239.0f, titleScale, glm::vec3(0.2f, 0.12f, 0.0f));

    renderText(title, titleX, 235.0f, titleScale, yellow);

    drawRectangle(190.0f, 315.0f, 420.0f, 2.0f, blue);

    const std::string hint{"CHOOSE YOUR DIRECTION"};
    constexpr float hintScale{2.5f};

    renderText(hint, centeredX(hint, hintScale), 340.0f, hintScale, lightGray);
}
...
void Hud::renderLevelComplete(const GameState& state)
{
    ...
    renderText(title, titleX + 4.0f, 139.0f, titleScale, glm::vec3(0.18f, 0.12f, 0.0f));
    ...
    renderText(level, centeredX(level, informationScale), 260.0f, informationScale, lightGray);

    const std::string score{"SCORE " + std::to_string(state.getScore())};

    renderText(score, centeredX(score, informationScale), 310.0f, informationScale, yellow);
    ...
    renderText(prompt, centeredX(prompt, promptScale), 414.0f, promptScale, lightGray);
}
...
void Hud::renderPause(int selected)
{
    ...
    drawRectangle(110.0f, 55.0f, 580.0f, 490.0f, blue);

    drawRectangle(114.0f, 59.0f, 572.0f, 482.0f, panelColor);

    const std::string title{"PAUSED"};
    constexpr float titleScale{8.0f};
    const float titleX = centeredX(title, titleScale);

    renderText(title, titleX + 4.0f, 94.0f, titleScale, darkBlue);

    renderText(title, titleX, 90.0f, titleScale, yellow);

    const std::string subtitle{"GAME PAUSED"};
    constexpr float subtitleScale{2.0f};

    renderText(subtitle, centeredX(subtitle, subtitleScale), 160.0f, subtitleScale, lightGray);

    drawRectangle(155.0f, 205.0f, 490.0f, 215.0f, darkBlue);

    drawRectangle(159.0f, 209.0f, 482.0f, 207.0f, glm::vec3(0.012f, 0.03f, 0.09f));
    ...
    for (int option = 0; option < 2; option++)
    {
        ...
        if (isSelected)
        {
            drawRectangle(182.0f, optionY - 13.0f, 436.0f, 58.0f, darkBlue);

            drawRectangle(182.0f, optionY - 13.0f, 436.0f, 2.0f, yellow);

            drawRectangle(182.0f, optionY + 43.0f, 436.0f, 2.0f, yellow);
        }
    ...
    renderText(navigationHint, centeredX(navigationHint, informationScale), 447.0f,
               informationScale, lightGray);

    renderText(resumeHint, centeredX(resumeHint, informationScale), 477.0f, informationScale, blue);
...
void Hud::renderScoreboard(const Scoreboard& scoreboard)
{
    ...
    renderText(title, centeredX(title, titleScale), 42.0f, titleScale, yellow);

    drawRectangle(105.0f, 122.0f, 590.0f, 356.0f, blue);

    drawRectangle(108.0f, 125.0f, 584.0f, 350.0f, panelColor);

    constexpr float rowScale{3.0f};

    renderText("RANK", 192.0f - textWidth("RANK", rowScale), 140.0f, rowScale, blue);

    renderText("PLAYER", 230.0f, 140.0f, rowScale, blue);

    renderText("SCORE", 660.0f - textWidth("SCORE", rowScale), 140.0f, rowScale, blue);

    drawRectangle(125.0f, 169.0f, 550.0f, 3.0f, blue);
    ...
    else
    {
        for (std::size_t i = 0; i < entries.size() && i < 10; i++)
        {
            ...
            if (i % 2 == 0)
            {
                drawRectangle(120.0f, rowY - 3.0f, 560.0f, 27.0f, glm::vec3(0.035f, 0.065f, 0.17f));
            }
    ...
    drawRectangle(buttonX, buttonY, buttonWidth, buttonHeight, glm::vec3(0.02f, 0.08f, 0.22f));

    drawRectangle(buttonX, buttonY, buttonWidth, borderThickness, blue);

    drawRectangle(buttonX, buttonY + buttonHeight - borderThickness, buttonWidth, borderThickness,
                  blue);

    drawRectangle(buttonX, buttonY, borderThickness, buttonHeight, blue);

    drawRectangle(buttonX + buttonWidth - borderThickness, buttonY, borderThickness, buttonHeight,
                  blue);

    const std::string returnButton{"> RETURN TO MENU <"};
    constexpr float buttonScale{3.0f};

    renderText(returnButton, centeredX(returnButton, buttonScale), 526.0f, buttonScale, yellow);
}
...
void Hud::renderGameOver(const GameState& state)
{
    ...
    drawRectangle(90.0f, 65.0f, 620.0f, 470.0f, red);

    drawRectangle(94.0f, 69.0f, 612.0f, 462.0f, panelColor);

    const std::string title{"GAME OVER"};
    constexpr float titleScale{8.0f};
    const float titleX = centeredX(title, titleScale);

    renderText(title, titleX + 4.0f, 109.0f, titleScale, darkRed);

    renderText(title, titleX, 105.0f, titleScale, red);

    drawRectangle(135.0f, 188.0f, 530.0f, 3.0f, darkRed);

    const std::string scoreLabel{"FINAL SCORE"};
    constexpr float labelScale{3.0f};

    renderText(scoreLabel, centeredX(scoreLabel, labelScale), 218.0f, labelScale, lightGray);

    const std::string score = std::to_string(state.getScore());
    constexpr float scoreScale{6.0f};

    renderText(score, centeredX(score, scoreScale), 258.0f, scoreScale, yellow);

    drawRectangle(230.0f, 325.0f, 340.0f, 2.0f, darkRed);

    const std::string levelLabel{"LEVEL REACHED"};

    renderText(levelLabel, centeredX(levelLabel, labelScale), 352.0f, labelScale, lightGray);

    const std::string level = std::to_string(state.getLevel());
    constexpr float levelScale{5.0f};

    renderText(level, centeredX(level, levelScale), 390.0f, levelScale, red);
    ...
    drawRectangle(promptX, promptY, promptWidth, promptHeight, darkRed);

    drawRectangle(promptX, promptY, promptWidth, borderThickness, red);

    drawRectangle(promptX, promptY + promptHeight - borderThickness, promptWidth, borderThickness,
                  red);

    drawRectangle(promptX, promptY, borderThickness, promptHeight, red);

    drawRectangle(promptX + promptWidth - borderThickness, promptY, borderThickness, promptHeight,
                  red);

    const std::string prompt{"PRESS ENTER TO CONTINUE"};
    constexpr float promptScale{2.5f};

    renderText(prompt, centeredX(prompt, promptScale), 478.0f, promptScale, yellow);
}
...
void Hud::renderHighScore(const std::string& playerName, int score, bool isNewHighestScore)
{
    ...
    const glm::vec3 darkAccent = isNewHighestScore ? darkGold : darkBlue;
    ...
    drawRectangle(55.0f, 25.0f, 690.0f, 550.0f, accent);

    drawRectangle(59.0f, 29.0f, 682.0f, 542.0f, panelColor);
    ...
    drawRectangle(270.0f, 145.0f, 260.0f, 100.0f, darkAccent);

    drawRectangle(273.0f, 148.0f, 254.0f, 94.0f, glm::vec3(0.015f, 0.03f, 0.085f));
    ...
    renderText(scoreText, centeredX(scoreText, scoreScale), 190.0f, scoreScale, gold);
    ...
    drawRectangle(inputX, inputY, inputWidth, inputHeight, accent);

    drawRectangle(inputX + 3.0f, inputY + 3.0f, inputWidth - 6.0f, inputHeight - 6.0f,
    ...
    for (std::size_t i = 0; i < maxNameLength; i++)
    {
        ...
        drawRectangle(slotX, 392.0f, slotWidth, 2.0f, mutedGray);

        if (i < visibleName.size())
        {
            renderText(std::string(1, visibleName[i]), slotX, 342.0f, nameScale, lightGray);
        }
    }
    ...
    renderText(characterHint, centeredX(characterHint, hintScale), 455.0f, hintScale, mutedGray);
    ...
    drawRectangle(saveButtonX, saveButtonY, saveButtonWidth, saveButtonHeight, darkAccent);

    drawRectangle(saveButtonX, saveButtonY, saveButtonWidth, borderThickness, accent);

    drawRectangle(saveButtonX, saveButtonY + saveButtonHeight - borderThickness, saveButtonWidth,
                  borderThickness, accent);

    drawRectangle(saveButtonX, saveButtonY, borderThickness, saveButtonHeight, accent);

    drawRectangle(saveButtonX + saveButtonWidth - borderThickness, saveButtonY, borderThickness,
                  saveButtonHeight, accent);

    const std::string savePrompt{"> PRESS ENTER TO SAVE <"};
    constexpr float savePromptScale{2.5f};

    renderText(savePrompt, centeredX(savePrompt, savePromptScale), 499.0f, savePromptScale, gold);

    const std::string deleteHint{"BACKSPACE TO DELETE"};

    renderText(deleteHint, centeredX(deleteHint, hintScale), 548.0f, hintScale, mutedGray);
}
