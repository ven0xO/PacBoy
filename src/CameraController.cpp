...
)
{
    ...
    const glm::vec2 playerDirection =
        player.getCameraDirection();
    ...
    if (glm::length(playerDirection) > 0.01f)
    {
        cameraOffset = glm::vec3(
            -playerDirection.x * 10.0f,
            5.0f,
            -playerDirection.y * 10.0f
        );
    }

    const glm::vec3 targetPosition =
        lookTarget + cameraOffset;

    const float followFactor =
        1.0f -
        std::exp(-CAMERA_FOLLOW_SPEED * deltaTime);
