...
namespace
{
    ...
    void testTargetingRules()
    {
        ...
        require(isNear(red.find_target(), glm::vec2(20.0f, 20.0f)), "red targeting changed");
        require(isNear(pink.find_target(), glm::vec2(23.0f, 20.0f)), "pink targeting changed");
        require(isNear(blue.find_target(), glm::vec2(39.0f, 35.0f)), "blue targeting changed");

        orange.set_position(glm::vec2(20.0f, 20.0f));
        require(isNear(orange.find_target(), glm::vec2(0.0f, static_cast<float>(grid.getHeight()))),
    ...
    void testReleaseDelays()
    {
        ...
        for (std::size_t index = 0; index < types.size(); ++index)
        {
            ...
            if (delays[index] > 0.0f)
            {
                ...
                require(isNear(enemy.get_position(), start), "ghost left before its release delay");
            }
    ...
    void testScaredState()
    {
        ...
        require(enemy.get_state() == State::Scared, "Scared ended too early");

        enemy.update(6.0f, 1, 0.05f);
        require(enemy.get_state() == State::Scatter, "ghost did not restore its active mode");
        ...
        require(enemy.get_state() == State::Chase, "mode schedule threshold changed");
    }
    ...
    void testSharedGameOperations()
    {
        ...
        {
            require(enemy.get().get_state() == State::Scatter, "ghost collection is incomplete");
        }
...
int main()
{
    ...
    {
        std::cerr << "Enemy regression test failed: " << error.what() << '\n';
        return 1;
    }
